 ...
 */
import { SocketErrorCode } from '../types/socket-events.js';
/**
...
export class RoomManager {
    ...
    joinRoom(roomCode, playerName, socketId) {
        ...
        if (room.status !== 'waiting') {
            throw this.createError(SocketErrorCode.GAME_IN_PROGRESS, 'Cannot join room while game is in progress');
        }
        const now = Date.now();
        const player = {
            id: socketId,
     ...
     */
    createSession(socketId, roomCode, player) {
        const now = Date.now();
        const session = {
            socketId,
            roomCode,
     ...
     */
    restoreSession(previousSocketId, newSocketId, roomCode) {
